...
locations=".travis.yml "$(find . \( -name '*.py' -or -name '*.sh' -or -name '*.md' -or -name '*.rst' -or -name '*.txt' -or -name '*.ini' \) ! \( -name code.py -or -name TODO.sh -or -name 'UNRELEASED*' -or -path '*dist*' -or -path '*build*' -or -path '*egg*' \))
